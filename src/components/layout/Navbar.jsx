  ...
  return (
    ...
    <motion.nav
      className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${
        isScrolled ? 'bg-white/95 dark:bg-dark/95 backdrop-blur-md shadow-lg' : 'bg-black/20 md:bg-transparent'
      }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
    ...
    >
      <div className="max-w-[75rem] mx-auto px-6 py-4 flex items-center justify-between">
        <Link to={`/`} className={`flex items-center gap-2 transition-colors ${linkColor} dark:text-white`}>
          <img src={`${import.meta.env.BASE_URL}logo.png`} alt="Unique Visuals" className="h-12 md:h-14 w-auto" />
        </Link>
