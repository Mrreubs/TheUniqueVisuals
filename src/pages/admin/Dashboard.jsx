        ...
        {data.map((item, idx) => {
          ...
          return (
            <div key={idx} className="flex-1 flex flex-col items-center gap-2 group">
              ...
              <span className="text-xs text-gray-600 dark:text-white/60 truncate w-full text-center">{item.label}</span>
            </div>
          );
        })}
      </div>
...
export default function Dashboard() {
  ...
  const chartData = [
    ...
    { label: 'May', value: 89 }, { label: 'Jun', value: 72 }
  ];
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (activeTab === 'analytics' || activeTab === 'content') {
        ...
        const testimonialsSnap = await getDocs(testQ);
        setStats(prev => ({ ...prev, testimonials: testimonialsSnap.size }));
  ...
  const handleDeleteUser = async (id) => {
    if (!window.confirm("Delete this user? (Warning: This only deletes the Firestore doc, not their Auth account)")) return;
    try {
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          >
            ...
            <nav className="space-y-2">
              <button onClick={() => setActiveTab('analytics')} className={`w-full flex items-center gap-3 px-6 py-4 rounded-xl transition-all duration-300 ${activeTab === 'analytics' ? 'bg-gold text-white shadow-md' : 'hover:bg-gray-50 dark:hover:bg-white/5 text-gray-600 dark:text-white/60 hover:text-gray-900 dark:hover:text-white hover:translate-x-1'}`}>
                <BarChart3 size={18} /> Analytics
              </button>
              <button onClick={() => setActiveTab('portfolio')} className={`w-full flex items-center gap-3 px-6 py-4 rounded-xl transition-all duration-300 ${activeTab === 'portfolio' ? 'bg-gold text-white shadow-md' : 'hover:bg-gray-50 dark:hover:bg-white/5 text-gray-600 dark:text-white/60 hover:text-gray-900 dark:hover:text-white hover:translate-x-1'}`}>
                <ImageIcon size={18} /> Portfolio Upload
              </button>
              <button onClick={() => setActiveTab('users')} className={`w-full flex items-center gap-3 px-6 py-4 rounded-xl transition-all duration-300 ${activeTab === 'users' ? 'bg-gold text-white shadow-md' : 'hover:bg-gray-50 dark:hover:bg-white/5 text-gray-600 dark:text-white/60 hover:text-gray-900 dark:hover:text-white hover:translate-x-1'}`}>
                <Users size={18} /> Clients
              </button>
              <button onClick={() => setActiveTab('content')} className={`w-full flex items-center gap-3 px-6 py-4 rounded-xl transition-all duration-300 ${activeTab === 'content' ? 'bg-gold text-white shadow-md' : 'hover:bg-gray-50 dark:hover:bg-white/5 text-gray-600 dark:text-white/60 hover:text-gray-900 dark:hover:text-white hover:translate-x-1'}`}>
                <MessageSquare size={18} /> Reviews
              </button>
              <div className="pt-8">
                <button onClick={handleLogout} className="w-full flex items-center gap-3 px-6 py-4 rounded-xl hover:bg-red-500/10 hover:text-red-500 text-gray-600 dark:text-white/60 transition-colors duration-300">
          ...
          >
            ...
            {activeTab === 'analytics' && (
              <div className="space-y-8 animate-in fade-in duration-500">
                <div className="flex items-center justify-between">
                  ...
                  {loadingData && <div className="w-5 h-5 border-2 border-gold border-t-transparent rounded-full animate-spin"></div>}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 p-6 rounded-3xl hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                    ...
                    <h4 className="text-3xl font-medium mb-1">{stats.users}</h4>
                    <p className="text-gray-600 dark:text-white/60 text-sm">Registered Clients</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 p-6 rounded-3xl hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                    <div className="flex justify-between items-start mb-4">
                    ...
                    <h4 className="text-3xl font-medium mb-1">{stats.bookings}</h4>
                    <p className="text-gray-600 dark:text-white/60 text-sm">Total Bookings Recorded</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 p-6 rounded-3xl hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                    <div className="flex justify-between items-start mb-4">
                    ...
                    <h4 className="text-3xl font-medium mb-1">{stats.testimonials}</h4>
                    <p className="text-gray-600 dark:text-white/60 text-sm">Testimonials Submitted</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <CustomBarChart data={chartData} title="Monthly Site Visitors (Demo)" />
                  <CustomBarChart data={categoryViewsData} title="Views by Category (Demo)" />
                </div>
              </div>
            )}
            ...
            {activeTab === 'portfolio' && (
              <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <p className="text-gray-600 dark:text-white/60 mb-8">Upload new images to your portfolio categories. They will be saved to Firebase Storage.</p>

                <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 p-8 rounded-3xl max-w-2xl">
                  <form onSubmit={handleUploadSubmit} className="space-y-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-600 dark:text-white/60 mb-2">Select Category</label>
                      <select 
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-600 dark:text-white/60 mb-2">Upload Image</label>
                      <div className="border-2 border-dashed border-gray-300 dark:border-white/20 rounded-2xl p-12 text-center hover:border-gold transition-colors relative group">
                        ...
                        <p className="text-gray-900 dark:text-white font-medium mb-1">Click to browse or drag and drop</p>
              ...
              <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-3xl overflow-hidden">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                      {usersList.length === 0 ? (
                        <tr><td colSpan="4" className="p-6 text-center text-gray-600 dark:text-white/60">No users found or Firebase not configured.</td></tr>
                      ) : usersList.map((user) => (
                        <tr key={user.id} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                          <td className="p-4 text-gray-900 dark:text-white font-medium">{user.displayName || 'Unknown'}</td>
