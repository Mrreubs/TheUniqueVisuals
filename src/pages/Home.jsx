    ...
    <div className="bg-white dark:bg-dark min-h-screen">
        ...
        <div className="absolute bottom-10 left-0 right-0 z-20 flex justify-center gap-3">
          {HERO_IMAGES.map((_, index) => (
            <button
              ...
              className={`h-3 min-w-[44px] transition-all duration-300 rounded-full ${
                currentSlide === index ? 'w-12 bg-gold' : 'w-3 bg-white/40'
              }`}
      ...
      <section className="py-24 px-6 max-w-[75rem] mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
        ...
        >
          ...
          <p className="text-gray-700 dark:text-white/70 text-lg leading-relaxed mb-8">
            At Unique Visuals, we believe every picture tells a story. Based in Ibadan, we travel across Nigeria to
            capture your most precious moments with an elite, editorial touch. Our passion is to make you look and feel
            extraordinary.
          </p>
          <Link
            to="/about"
            className="inline-flex items-center gap-2 text-gray-900 dark:text-white hover:text-gold uppercase tracking-widest text-sm font-bold transition-colors group"
          >
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
        >
          <img src={`${BASE}images/_mg_1526.webp`} alt="" className="object-cover w-full h-full" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {SERVICES.map((service, index) => (
              ...
              >
                ...
                <Link
                  to="/services"
                  className="text-gold text-sm font-bold uppercase tracking-widest hover:text-gray-900 dark:hover:text-white transition-colors"
                >
                  Learn More
                </Link>
              </motion.div>
          ...
          {PORTFOLIO_PREVIEW.map((item) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center">
                <span className="text-white font-display text-xl md:text-2xl font-bold tracking-wider">
                  {item.category}
                </span>
              </div>
            </motion.div>
      ...
      <section className="py-24 bg-gradient-to-b from-gray-50 to-white dark:from-dark-secondary dark:to-dark">
        <div className="max-w-[75rem] mx-auto px-6 text-center">
          ...
          <h2 className="text-3xl md:text-5xl font-display font-bold text-gray-900 dark:text-white mb-12 max-w-3xl mx-auto leading-tight">
            &ldquo;Unique Visuals gave us exactly what we wanted for our wedding. The pictures look like they belong in a
            magazine!&rdquo;
          </h2>
          ...
          <div className="mt-12">
            <Link
              to="/testimonials"
              className="text-gray-900 dark:text-white hover:text-gold border border-gray-300 dark:border-white/20 hover:border-gold hover:bg-gray-50 dark:hover:bg-white/5 px-8 py-4 rounded text-sm font-bold uppercase tracking-widest transition-colors inline-block"
            >
              Read More Reviews
            </Link>
          </div>
      ...
      <section className="py-32 px-6 relative overflow-hidden">
        ...
        <div
          ...
          style={{ backgroundImage: `url(${BASE}images/_mg_0970.webp)` }}
        />
